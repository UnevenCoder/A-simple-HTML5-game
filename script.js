...
const checkFail = () => {
  if (ballY.get() >= 0 && ballY.getVelocity() !== 0 && ball.innerHTML !== 'Tap') {
    ...
    ball.innerHTML = 'Tap';
    document.getElementById('s').innerHTML='Score : 0'
    document.getElementsByClassName('ball')[0].style.backgroundColor='#9B65DE'
  }
};
...
listen(ball, 'mousedown touchstart').start((e) => {
  ...
  document.getElementById('s').innerHTML='Score : '+count
  if(count > parseInt(localStorage.getItem('highscore'))){
    localStorage.setItem('highscore',count)
    document.getElementById('hs').innerHTML='High Score : '+count
  }
  //if(count%10==0){
    document.getElementsByClassName('ball')[0].style.backgroundColor=randomColors()
 // }
  isFalling = true;
  ballScale.stop();
  ballScale.update(1);
